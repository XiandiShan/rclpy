...
ActionGoalHandle::ActionGoalHandle(
  rclpy_debug::ActionServer & action_server, py::object pygoal_info_msg)
: action_server_(action_server)
{
  ...
  if (!rcl_handle) {
    throw rclpy_debug::RCLError("Failed to accept new goal");
  }
...
ActionGoalHandle::get_status()
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to get goal status");
  }
...
ActionGoalHandle::update_goal_state(rcl_action_goal_event_t event)
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to update goal state");
  }
}
...
define_action_goal_handle(py::module module)
{
  ...
  .def(py::init<rclpy_debug::ActionServer &, py::object>())
