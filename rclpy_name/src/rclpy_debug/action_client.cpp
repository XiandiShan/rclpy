...
#define SEND_SERVICE_REQUEST(Type) \
  ...
  if (RCL_RET_OK != ret) { \
    throw rclpy_debug::RCLError("Failed to send " #Type " request"); \
  } \
...
#define TAKE_MESSAGE(Type) \
  ...
  if (RCL_RET_OK != ret) { \
    ...
    throw rclpy_debug::RCLError("Failed to take " #Type " with an action client"); \
  } \
...
ActionClient::get_num_entities()
{
  ...
  if (RCL_RET_OK != ret) {
    ...
    throw rclpy_debug::RCLError(error_text);
  }
...
ActionClient::is_action_server_available()
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to check if action server is available");
  }
  return is_available;
}
...
ActionClient::is_ready(WaitSet & wait_set)
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to get number of ready entities for action client");
  }
