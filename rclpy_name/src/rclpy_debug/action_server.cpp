...
ActionServer::ActionServer(
  Node & node,
  const rclpy_debug::Clock & rclpy_clock,
...
#define TAKE_SERVICE_REQUEST(Type) \
  ...
  if (ret == RCL_RET_ACTION_CLIENT_TAKE_FAILED || ret == RCL_RET_ACTION_SERVER_TAKE_FAILED) { \
    ...
    throw rclpy_debug::RCLError("Failed to take " #Type); \
  } \
...
#define SEND_SERVICE_RESPONSE(Type) \
  ...
  if (RCL_RET_OK != ret) { \
    throw rclpy_debug::RCLError("Failed to send " #Type " response"); \
  }
...
ActionServer::publish_feedback(py::object pymsg)
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to publish feedback with an action server");
  }
}
...
ActionServer::publish_status()
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed get goal status array");
  }
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed publish goal status array");
  }
}
...
ActionServer::notify_goal_done()
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to notfiy action server of goal done");
  }
}
...
ActionServer::get_num_entities()
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to get number of entities for 'rcl_action_server_t'");
  }
...
ActionServer::is_ready(WaitSet & wait_set)
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to get number of ready entities for action server");
  }
...
ActionServer::add_to_waitset(WaitSet & wait_set)
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to add 'rcl_action_server_t' to wait set");
  }
}
...
ActionServer::expire_goals(int64_t max_num_goals)
{
  ...
  if (RCL_RET_OK != ret) {
    throw rclpy_debug::RCLError("Failed to expire goals");
  }
...
define_action_server(py::object module)
{
  ...
  .def(
    py::init<Node &, const rclpy_debug::Clock &, py::object, const char *,
