...
class TimeJump:
    ...
    def __init__(self, clock_change, delta):
        if not isinstance(clock_change, ClockChange):
            raise TypeError('clock_change must be an instance of rclpy_debug.clock.ClockChange')
...
class Clock:
    ...
    def create_jump_callback(self, threshold, *, pre_callback=None, post_callback=None):
        ...
        :rtype: :class:`rclpy_debug.clock.JumpHandler`
        """
...
class ROSClock(Clock):
    ...
    def set_ros_time_override(self, time: Time):
        if not isinstance(time, Time):
            raise TypeError(
                'Time must be specified as rclpy_debug.time.Time. Received type: {0}'.format(type(time)))
        with self.handle:
