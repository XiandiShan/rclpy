...
def _declare_qos_parameters(
    ...
    :param entity_type: Either `rclpy_debug.node.Publisher` or `rclpy_debug.node.Subscription`.
...
def _override_qos_policy_with_param(qos: QoSProfile, policy: QoSPolicyKind, param: Parameter):
    ...
    if policy in (
        ...
        # e.g. `policy=QosPolicyKind.LIVELINESS` -> `policy_enum_class=rclpy_debug.qos.LivelinessPolicy`
        policy_enum_class = getattr(
            rclpy_debug.qos, f'{capitalize_first_letter(policy_name)}Policy')
        try:
