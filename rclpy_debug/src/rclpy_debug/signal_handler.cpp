...
wait_for_signal()
{
  ...
  switch (dw_wait_result) {
    ...
    case WAIT_TIMEOUT:
      RCUTILS_LOG_ERROR_NAMED(
        "rclpy_debug.signals", "WaitForSingleObject() timedout out in wait_for_signal()");
      break;
    case WAIT_FAILED:
      RCUTILS_LOG_ERROR_NAMED(
        "rclpy_debug.signals", "WaitForSingleObject() failed in wait_for_signal(): %s", GetLastError());
      break;
    default:
      RCUTILS_LOG_ERROR_NAMED(
        "rclpy_debug.signals", "WaitForSingleObject() gave unknown return in wait_for_signal(): %s",
        GetLastError());
  }
#elif defined(__APPLE__)
...
struct CleanupdeferredSignalHandler
{
  ...
  {
    // just in case nobody calls rclpy_debug.shutdown()
    teardown_deferred_signal_handler();
  }
};
...
/// Install rclpy_debug signal handlers.
/**
 * \param options rclpy_debug.signals.SignalHandlerOptions integer value.
 */
void
...
/**
 * \return rclpy_debug.signals.SignalHandlerOptions converted to integer value.
 */
...
define_signal_handler_api(py::module m)
{
  ...
  m.def(
    "register_sigint_guard_condition", &rclpy_debug::register_sigint_guard_condition,
    "Register a guard condition to be called on SIGINT.");
  m.def(
    "unregister_sigint_guard_condition", &rclpy_debug::unregister_sigint_guard_condition,
    "Stop triggering a guard condition when SIGINT occurs.");
  m.def(
    "install_signal_handlers", &rclpy_debug::install_signal_handlers,
    "Install rclpy_debug signal handlers.");
  m.def(
    "get_current_signal_handlers_options", &rclpy_debug::get_current_signal_handlers_options,
    "Get currently installed signal handler options.");
  m.def(
    "uninstall_signal_handlers", &rclpy_debug::uninstall_signal_handlers,
    "Uninstall rclpy_debug signal handlers.");
