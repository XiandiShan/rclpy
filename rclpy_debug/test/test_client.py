...
class TestClient(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.node = rclpy_debug.create_node('TestClient', context=cls.context)
    ...
    def test_concurrent_calls_to_service(self):
        ...
        try:
            ...
            rclpy_debug.spin_until_future_complete(self.node, future1, executor=executor)
            rclpy_debug.spin_until_future_complete(self.node, future2, executor=executor)
    ...
    def test_different_type_raises(self):
        ...
        try:
            ...
            executor = rclpy_debug.executors.SingleThreadedExecutor(context=self.context)
            with self.assertRaises(TypeError):
                rclpy_debug.spin_until_future_complete(self.node, future, executor=executor)
        finally:
            self.node.destroy_client(cli)
            self.node.destroy_service(srv)
