...
class TestExecutor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node = rclpy_debug.create_node('TestExecutor', namespace='/rclpy_debug', context=self.context)
    ...
    def test_global_executor_completes_async_task(self):
        ...
        executor = SingleThreadedExecutor(context=self.context)
        rclpy_debug.spin_once(self.node, timeout_sec=0.5, executor=executor)
        self.assertTrue(did_callback)

        timer.cancel()
        trigger.do_yield = False
        rclpy_debug.spin_once(self.node, timeout_sec=0, executor=executor)
