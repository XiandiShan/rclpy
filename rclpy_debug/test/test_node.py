...
class TestNodeAllowUndeclaredParameters(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.context = rclpy_debug.context.Context()
        rclpy_debug.init(context=cls.context)
        cls.node = rclpy_debug.create_node(
    ...
    def tearDownClass(cls):
        ...
        rclpy_debug.shutdown(context=cls.context)
    ...
    def test_node_logger(self):
        ...
        node_logger.set_level(rclpy_debug.logging.LoggingSeverity.INFO)
        node_logger.debug('test')

    def test_initially_no_executor(self):
        node = rclpy_debug.create_node('my_node', context=self.context)
        try:
    ...
    def test_node_cannot_declare_after_set(self):
        self.assertTrue(self.node.set_parameters([Parameter('my_param', value=5)])[0].successful)
        self.assertEqual(self.node.get_parameter('my_param').value, 5)
        with pytest.raises(rclpy_debug.exceptions.ParameterAlreadyDeclaredException):
...
class TestNode(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node = rclpy_debug.create_node(
            TEST_NODE,
    ...
    def test_use_sim_time(self):
        ...
        temp_node = rclpy_debug.create_node(
            TEST_NODE + '2',
            namespace=TEST_NAMESPACE,
            context=self.context,
            parameter_overrides=[
    ...
    def test_node_set_parameters(self):
        ...
        # Create rclpy_debug.Parameter list from tuples.
        parameters = [
            Parameter(
...
class TestCreateNode(unittest.TestCase):
    ...
    def test_node_get_fully_qualified_name(self):
        ...
        ns = '/my_ns'
        name = 'my_node'
        node = rclpy_debug.create_node(name, namespace=ns, context=context)
        assert node.get_fully_qualified_name() == '{}/{}'.format(ns, name)
        node.destroy_node()

        # When ns is not specified, a leading / should be added
        node_without_ns = rclpy_debug.create_node(name, context=context)
        ...
        node_with_remapped_ns = rclpy_debug.create_node(
            name,
            namespace=ns,
            context=context,
        ...
        node_with_remapped_name = rclpy_debug.create_node(
            name,
            namespace=ns,
            context=context,
        ...
        node_with_remapped_ns_name = rclpy_debug.create_node(
            name,
            namespace=ns,
            context=context,
        ...
        g_context = rclpy_debug.context.Context()
        global_remap_name = 'global_node_name'
        rclpy_debug.init(
            args=['--ros-args', '-r', '__node:=' + global_remap_name],
            context=g_context,
        )
        node_with_global_arguments = rclpy_debug.create_node(
            name,
            namespace=ns,
            context=g_context,
        )
        ...
        node_skip_global_params = rclpy_debug.create_node(
            name,
            namespace=ns,
            context=g_context,
        ...
        rclpy_debug.shutdown(context=g_context)


def test_node_resolve_name():
    context = rclpy_debug.Context()
    rclpy_debug.init(
        args=['--ros-args', '-r', 'foo:=bar'],
        context=context,
    )
    node = rclpy_debug.create_node('test_rclpy_node_resolve_name', namespace='/my_ns', context=context)
