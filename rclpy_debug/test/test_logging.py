...
class TestLogging(unittest.TestCase):
    ...
    def test_root_logger_level(self):
        original_severity = rclpy_debug.logging._root_logger.get_effective_level()
        for severity in LoggingSeverity:
            rclpy_debug.logging._root_logger.set_level(severity)
            self.assertEqual(
                severity, rclpy_debug.logging._root_logger.get_effective_level())
        rclpy_debug.logging._root_logger.set_level(original_severity)
    ...
    def test_logger_level(self):
        ...
        original_severity = rclpy_debug.logging.get_logger_effective_level(name)
        for severity in LoggingSeverity:
            if severity is not LoggingSeverity.UNSET:  # unset causes the hierarchy to be traversed
                rclpy_debug.logging.set_logger_level(name, severity)
                self.assertEqual(
                    severity, rclpy_debug.logging.get_logger_effective_level(name))
        rclpy_debug.logging.set_logger_level(name, original_severity)

    def test_logger_object_level(self):
        logger = rclpy_debug.logging.get_logger('test_logger')
        for severity in LoggingSeverity:
            if severity is not LoggingSeverity.UNSET:  # unset causes the hierarchy to be traversed
    ...
    def test_logger_effective_level(self):
        ...
        self.assertEqual(
            rclpy_debug.logging._root_logger.get_effective_level(),
            rclpy_debug.logging.get_logger_effective_level(name))

        # Check that the effective threshold for a logger with manually unset severity is default
        rclpy_debug.logging.set_logger_level(name, LoggingSeverity.UNSET)
        self.assertEqual(
            rclpy_debug.logging._root_logger.get_effective_level(),
            rclpy_debug.logging.get_logger_effective_level(name))
        # Check that the effective threshold for a logger with set severity
        rclpy_debug.logging.set_logger_level(name, LoggingSeverity.ERROR)
        self.assertEqual(
            LoggingSeverity.ERROR,
            rclpy_debug.logging.get_logger_effective_level(name))

    def test_log_threshold(self):
        rclpy_debug.logging._root_logger.set_level(LoggingSeverity.INFO)

        # Logging below threshold not expected to be logged
        self.assertFalse(rclpy_debug.logging._root_logger.debug('message_debug'))

        # Logging at or above threshold expected to be logged
        self.assertTrue(rclpy_debug.logging._root_logger.info('message_info'))
        self.assertTrue(rclpy_debug.logging._root_logger.warning('message_warn'))
        self.assertTrue(rclpy_debug.logging._root_logger.error('message_error'))
        self.assertTrue(rclpy_debug.logging._root_logger.fatal('message_fatal'))

    def test_log_once(self):
        message_was_logged = []
        for i in range(5):
            message_was_logged.append(rclpy_debug.logging._root_logger.log(
    ...
    def test_log_arguments(self):
        ...
        with self.assertRaisesRegex(TypeError, 'required parameter .* not specified'):
            rclpy_debug.logging._root_logger.log(
                'message',
                LoggingSeverity.INFO,
                throttle_time_source_type=system_clock,
            )
    ...
    def test_named_logger(self):
        my_logger = rclpy_debug.logging.get_logger('my_logger')
        ...
        loggers = [my_logger, rclpy_debug.logging._root_logger]
    ...
    def test_named_logger_hierarchy(self):
        ...
        with self.assertRaisesRegex(ValueError, 'Logger name must not be empty'):
            my_logger = rclpy_debug.logging.get_logger('')
        ...
        rclpy_debug.logging._root_logger.set_level(default_severity)
    ...
    def test_logging_severity_from_string(self):
        for severity in rclpy_debug.logging.LoggingSeverity:
            self.assertEqual(
                rclpy_debug.logging.get_logging_severity_from_string(severity.name), severity)

    def test_nonexistent_logging_severity_from_string(self):
        with self.assertRaises(RuntimeError):
            rclpy_debug.logging.get_logging_severity_from_string('non_existent_severity')
    ...
    def test_get_logging_directory(self):
        ...
        log_dir = rclpy_debug.logging.get_logging_directory()
