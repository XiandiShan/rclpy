...
def test_zero_callback(period):
    ...
    try:
        node = rclpy_debug.create_node('test_timer_no_callback', context=context)
        try:
...
def test_number_callbacks(period):
    ...
    rclpy_debug.init(context=context)
    try:
        node = rclpy_debug.create_node('test_timer_number_callbacks', context=context)
        try:
            executor = SingleThreadedExecutor(context=context)
            try:
                executor.add_node(node)
                ...
                try:
                    ...
                    while rclpy_debug.ok(context=context) and time.time() - begin_time < 4.5 * period:
...
def test_cancel_reset(period):
    ...
    rclpy_debug.init(context=context)
    try:
        node = rclpy_debug.create_node('test_timer_cancel_reset', context=context)
        try:
            executor = SingleThreadedExecutor(context=context)
            try:
                executor.add_node(node)
                ...
                try:
                    ...
                    begin_time = time.time()
                    while rclpy_debug.ok(context=context) and time.time() - begin_time < 2.5 * period:
                        executor.spin_once(timeout_sec=period / 10)
...
def test_time_until_next_call():
    ...
    rclpy_debug.init(context=context)
    try:
        node = rclpy_debug.create_node('test_time_until_next_call', context=context)
        executor = SingleThreadedExecutor(context=context)
        executor.add_node(node)
        executor.spin_once(timeout_sec=0)
