...
class TestActionServer(unittest.TestCase):
    ...
    def setUp(self):
        self.context = rclpy_debug.context.Context()
        rclpy_debug.init(context=self.context)
        self.executor = SingleThreadedExecutor(context=self.context)
        self.node = rclpy_debug.create_node('TestActionServer', context=self.context)
        self.mock_action_client = MockActionClient(self.node)

    def tearDown(self):
        self.node.destroy_node()
        self.executor.shutdown()
        rclpy_debug.shutdown(context=self.context)

    def timed_spin(self, duration):
        start_time = time.time()
        while (time.time() - start_time) < duration:
            rclpy_debug.spin_once(self.node, executor=self.executor, timeout_sec=0.1)
    ...
    def test_constructor_no_defaults(self):
        action_server = ActionServer(
            ...
            goal_service_qos_profile=rclpy_debug.qos.QoSProfile(depth=10),
            result_service_qos_profile=rclpy_debug.qos.QoSProfile(depth=10),
            cancel_service_qos_profile=rclpy_debug.qos.QoSProfile(depth=10),
            feedback_pub_qos_profile=rclpy_debug.qos.QoSProfile(depth=10),
            status_pub_qos_profile=rclpy_debug.qos.QoSProfile(depth=10),
            result_timeout=300,
        )
        action_server.destroy()
    ...
    def test_single_goal_accept(self):
        ...
        rclpy_debug.spin_until_future_complete(self.node, future, self.executor)
    ...
    def test_multi_goal_accept(self):
        ...
        rclpy_debug.spin_until_future_complete(self.node, future0, executor)
        rclpy_debug.spin_until_future_complete(self.node, future1, executor)
        rclpy_debug.spin_until_future_complete(self.node, future2, executor)
    ...
    def test_cancel_goal_accept(self):
        ...
        rclpy_debug.spin_until_future_complete(self.node, goal_future, executor)
        ...
        rclpy_debug.spin_until_future_complete(self.node, cancel_future, executor)
    ...
    def test_cancel_defered_goal(self):
        ...
        goal_msg.goal_id = goal_uuid
        goal_future = self.mock_action_client.send_goal(goal_msg)
        rclpy_debug.spin_until_future_complete(self.node, goal_future, self.executor)
        ...
        cancel_srv.goal_info.stamp.nanosec = 0
        cancel_future = self.mock_action_client.cancel_goal(cancel_srv)
        rclpy_debug.spin_until_future_complete(self.node, cancel_future, self.executor)
        cancel_result = cancel_future.result()
        self.assertEqual(len(cancel_result.goals_canceling), 1)
        ...
        rclpy_debug.spin_until_future_complete(self.node, get_result_future, self.executor)
    ...
    def test_different_feedback_type_raises(self):
        ...
        try:
            ...
            rclpy_debug.spin_until_future_complete(
